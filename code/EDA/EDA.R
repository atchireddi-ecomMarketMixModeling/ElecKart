...
data_week <- data %>% group_by(week) %>% 
  summarise(gmv=sum(gmv),product_mrp=mean(product_mrp),
            ...
            n_saledays=mean(n_saledays),TV=mean(TV),Digital=mean(Digital),
            Sponsorship=mean(Sponsorship),ContentMarketing=mean(ContentMarketing),
            OnlineMarketing=mean(OnlineMarketing),Affiliates=mean(Affiliates),SEM=mean(SEM),
            Radio=mean(Radio),Other=mean(Other),TotalInvestment=mean(TotalInvestment),
            NPS=mean(NPS),list_mrp=mean(list_mrp),
            units=sum(units),COD=sum(COD),Prepaid=sum(Prepaid))
